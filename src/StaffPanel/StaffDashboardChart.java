 ...
 * @author luisr
 */
public class StaffDashboardChart extends javax.swing.JPanel {

    private int currentUserId = 1; 
    public StaffDashboardChart() {
        initComponents();
        initCharts();
    }
 ...
 private JFreeChart createRequestStatusChart() {
        ...
        if (data.isEmpty()) {
            // If no data, add default zero values
    ...
    private Map<String, Integer> getRequestStatusCounts(int userId) {
        ...
        try {
            ...
            String sql = "SELECT status, COUNT(*) AS count FROM request_tbl WHERE user_id = ? GROUP BY status";
            ...
            ModalCustom.showError("Invalid get requests: " + e.getMessage(), "Invalid");
        }
        return statusCount;
    }
    private Map<String, Integer> getRequestOverTime(int userId) {
        Map<String, Integer> requestOverTime= new java.util.LinkedHashMap<>();
        ...
        try {
            ...
            String sql = "SELECT "
                    ...
                    + "COUNT(*) AS count FROM request_tbl "
                    + "WHERE user_id = ? GROUP BY month ORDER BY month ASC";
            PreparedStatement pst = conn.prepareStatement(sql);
            pst.setInt(1,userId);
            ResultSet rs = pst.executeQuery();
            
            while (rs.next()) {
                requestOverTime.put(rs.getString("month"), rs.getInt("count"));
            }
            rs.close();
            pst.close();
            db.closeConnection();
        } catch (SQLException e) {
            ModalCustom.showError("Invalid charts loading: " + e.getMessage(), "Error");
        }
